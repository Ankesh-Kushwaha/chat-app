...
export const userLogin = async (req: Request, res: Response) => {
  try {
    ...
    if (!verify) return res.status(400).json("please provide valid data");

    const user = await User.findOne({ email });
    if (!user) return res.status(400).json("user does not exist");
    //verify the password with the user password
    const passwordVerification = await bcrypt.compare(password, user.password)
    if (!passwordVerification) return res.status(400).json("password is invalid");
    //generate  token 
    const createdUser = user.toObject() as { [key: string]: any };
    delete createdUser.password;
...
export const sendFriendRequest = async (req: Request, res: Response) => {
  try {
    ...
    if (senderId === receiverId) return res.status(400).json("can't add yourself");
    //find the user
    const sender = await User.findById(senderId);
    const receiver = await User.findById(receiverId);

    if (!receiver) return res.status(400).json("receiver does not exist");
    //check if user is already friend
    if (sender?.friends.includes(receiverId)) return res.status(400).json("user is already friends");
    ...
    if (pendingRequest) return res.status(400).json("friend request already exist");
...
export const acceptFriendRequest = async (req: Request, res: Response) => {
  try {
    ...
    const pendingRequest = await FriendRequest.findById(requestId)
      ...
      .populate<{ receiver: IUser }>("receiver");
    ...
    const sender = pendingRequest.sender as IUser;
    const receiver = pendingRequest.receiver as IUser;
...
export const declineArequest = async (req: Request, res: Response) => {
  try {
    logger.info("decline a request endpoint hit")
    const requestId = req.body;
    if (!requestId) return res.status(400).json("requestId is required");
    const request = await FriendRequest.findById(requestId);
    if (!request || request.status !== "pending") return res.status(200).json("request got expired");
